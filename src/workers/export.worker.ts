...
import { createExportJobHandler } from './exportJob';
...
const handle = createExportJobHandler({
  decodePng: decodePngToPyramid,
  post: (reply) => {
...
self.onmessage = (event: MessageEvent) => {
  ...
  if (request) {
    const jobId = 'jobId' in request ? request.jobId : 'unknown';
  ...
  const data = event.data as { jobId?: unknown } | null;
